...
export const main = (n, matrice, setLoading, setDrum) => {
  ...
  if (newL[1][1] === -1) {
    toast.error('Pozitia initiala este blocata!', { autoClose: 2000 })
    setLoading(true)
    setDrum([0, 0])

    return
  } else if (newL[n][n] === 99999999) {
    toast.error('Nu este nici un drum disponibil!', { autoClose: 2000 })
    setLoading(true)
    setDrum([0, 0])
    ...
    toast.success(`${"Drumul minim are costul "} ${newL[n][n]}`, { autoClose: 2000 })
  }
}